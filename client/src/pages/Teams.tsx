...
const teams = [
  { id: "1", name: "Engineering", owner: "Admin User", members: 8, budget: 500, monthlySpend: 234 },
  { id: "2", name: "Research", owner: "Sarah Researcher", members: 4, budget: 300, monthlySpend: 189 },
  { id: "3", name: "Data", owner: "Mike Analyst", members: 3, budget: 200, monthlySpend: 45 },
  { id: "4", name: "Support", owner: "Admin User", members: 6, budget: 150, monthlySpend: 67 },
];
...
export default function Teams() {
  ...
  const totalBudget = teams.reduce((a, t) => a + t.budget, 0);
  const totalSpend = teams.reduce((a, t) => a + t.monthlySpend, 0);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6">
      <div className="max-w-[1800px] mx-auto">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2 tracking-tight">Teams</h1>
            <p className="text-slate-400 text-lg">Manage teams and their budgets</p>
          </div>
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            ...
            <DialogContent className="bg-slate-800 border-slate-700">
              <DialogHeader>
                ...
                <DialogDescription>Set up a new team with budget limits.</DialogDescription>
              </DialogHeader>
              <div className="space-y-4 py-2">
                <div>
                  <Label className="text-slate-300">Name</Label>
                  <Input value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="My Team" className="bg-slate-700 border-slate-600 text-white mt-1" />
                </div>
                <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <Card className="bg-slate-800/30 border-slate-700/50 backdrop-blur">
            <CardContent className="p-6">
              ...
              <div className="text-3xl font-bold text-white">${totalSpend}</div>
            </CardContent>
          </Card>
        </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-slate-700/50 hover:bg-transparent">
                  ...
                  <TableHead className="text-slate-300">Monthly Spend</TableHead>
                  <TableHead className="text-slate-300">Usage</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {teams.map((team) => {
                  const pct = team.budget > 0 ? (team.monthlySpend / team.budget) * 100 : 0;
                  return (
                    <TableRow key={team.id} className="border-slate-700/50 hover:bg-slate-700/30">
                      ...
                      <TableCell className="text-slate-300">${team.budget}</TableCell>
                      <TableCell className="text-slate-300">${team.monthlySpend}</TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
